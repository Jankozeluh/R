...
get_closer_info <- function(product_link){
  ...
  open <- page %>% html_nodes(".auction-stats__tag+ .auction-stats__qty td") %>% html_text() %>% strptime(., "%b %d, %Y %I:%M:%S %p",tz="Europe/London")
  ...
  return(c(bidinc,qua,listin,skuu,open,listinT,met,descc))
}

closer_info = sapply(link, FUN = get_closer_info, USE.NAMES = FALSE)
offered_items <- tibble(Title = title, Desc = closer_info[8,] ,Start = closer_info[5,], End=time, CurrentPrice=price, NBids=n_of_bids, MetNot = closer_info[7,], ProductImg=product_img, Link=link, BidIncrement = closer_info[1,], Quantity = closer_info[2,],  Listing = closer_info[3,], SKU = closer_info[4,], ListingType = closer_info[6,])

write.csv2(offered_items , file = "~/Documents/R/f1/tracks_ex/dd.csv", row.names = FALSE)
